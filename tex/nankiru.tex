...
\usepackage{graphicx, expl3, minibox, amsmath, stfloats, tablefootnote, tikz, float, threeparttable, xfrac, multirow, etoolbox, verbatim, diagbox, siunitx, titling}
...
\begin{enumerate}
\item \verb+\citet[p.~100]{muhai}+
\item \verb+\citep[][p.~300]{tetsusenryaku}+
\item \verb+\multicitep{hirasawa-naki, p.~48; rb1, p.~216}+
\end{enumerate}

そうすると、引用が\citet[p.~100]{muhai}になる\citep[][p.~300]{tetsusenryaku}。複数の引用も\multicitep{hirasawa-naki, p.~48; rb1, p.~216}。
...
手牌\ref{mj:block-2}に打 \mj{6m} として6ブロックにする手もあるが、ツモ \mj{7p25s} での完全一向聴を逃さないように \mj{89s} を落とすのは現代流のセオリーであろう\citep[][p. 21]{ishibashi-black}。
...
6ブロックを5ブロックに絞る時に搭子の落とし順は「辺張＞嵌張＞対子＞両面」。例え、手牌\ref{mj:block-4}は対子の \mj{1m} を落とすのは効率が一番良く、手牌\ref{mj:block-5}なら \mj{12p} を払う\citep[][pp. 21--22]{ishibashi-black}。
...
\textbf{残り枚数が一枚減っていても打点が高い方にする}\citep[][pp. 91--92]{miinin-choice}。例え\ref{mj:rch-2-5}には嵌 \mj{6m} と嵌 \mj{4m} の二択がある。そして嵌 \mj{4m} にすれば一盃口がつく。残り4枚の方のアガリ率が高いけど、局収支として嵌 \mj{4m} の方が高い。
...
中張牌ドラ七対子の局収支は1400\citep[][p.~110]{miinin-choice}。表~\ref{tab:ev-chitoi} によると非ドラ生牌字牌（1700）より低いので生牌字牌にする\footnote{作者が実戦で出会ったケース。}。

\tehai{1105m 11p 11557s 334z}[東1局　南家　7巡目　ドラ \mj{7s}][]
...
表~\ref{tab:ev-gukei-chase} によると、親の立直が来て自分の手は愚形リーのみなら降りる。立直を含めて2飜があっても切る牌が危険なら降りてもいい。自分が親でのみ手から切る牌が無筋456なら降りる。その他の場合では押す。

愚形副露手なら状況が厳しくなる。表~\ref{tab:ev-gukei-furo} によると自分が子でも親でも立直に対して危険牌で1飜の手は押すべきではない。親の立直に対して2飜があっても切る牌が危険なら降りる。
...
\subsubsection*{安い副露手の押し引き}

\tehai*{2z 1s 4^z 5m 3'p 8^p 1^m 1^z 6^s 1^z 2^p 2^z}[対面（南家）の河][]

\tehai*{1m 5z 1z 8s 1s 3z 3z 7m 9p 8m 3s}[自家（北家）の河][]

\tehai{22345m 56p 567s -4'35m}[自分の手牌　ドラ \mj{7m}][3-8-1]

手牌\ref{mj:3-8-1}のように自分が1000点の聴牌。通ってない筋は \mj{3m}--\mj{6m}--\mj{9m}、\mj{1p}--\mj{4p}--\mj{7p}、\mj{2s}--\mj{5s}--\mj{8s} と \mj{4s}--\mj{7s}。以下は対面の河による通っていない筋を分析する\citep[][pp.~128--133]{tetsuoshi}。
