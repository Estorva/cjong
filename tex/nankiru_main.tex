...
\documentclass[10pt, twocolumn, notitlepage]{report}
...
\usepackage{graphicx, expl3, minibox, amsmath, stfloats, tablefootnote, tikz, float, threeparttable, xfrac, multirow, multicol, etoolbox, verbatim, diagbox, titling, subfiles, bibentry, amssymb, extramarks, lipsum, wrapfig}
\usetikzlibrary{backgrounds}

\usepackage[section]{placeins} %Float Barriers

% externalize rittai and use absolute paths
\usetikzlibrary{external}
\tikzsetexternalprefix{rittai-img/}
\tikzexternalize
...
\usepackage[nottoc]{tocbibind} % to add references into the ToC; use numbib to make it numbered in ToC
...
% remove space above chapter names
\makeatletter
...
% restore parskip in minipage
\makeatletter
\newcommand{\@minipagerestore}{\setlength{\parskip}{\baselineskip + 0pt plus 2pt\relax}}
\makeatother

\usepackage{xr}
% uncomment these when compiler main
...
\usepackage{fancyhdr}
\pagestyle{fancy}
% redefine \chaptermark and \sectionmark in the context of fancy page style
\renewcommand{\chaptermark}[1]{\markboth{#1}{}}
\renewcommand{\sectionmark}[1]{\markright{#1}}
% clear header and footnote
\fancyhf{}
% use " \ " to introduce two space characters to match the spacing
% between (sub)section number and (sub)section name
...
% make subsubsections numbered in report doc class
\setcounter{secnumdepth}{5}

% make ToC include subsubsections in report doc class
\setcounter{tocdepth}{3}

% remove the indentation of toc entries
\usepackage{tocloft}
...
% close the gap between sections and chapters
\renewcommand\cftchapafterpnum{\vskip-10pt}

% stop paragraphs from stretching over a page
\raggedbottom
...
% new env and command for text with rittai on the right
% \begin{textwrittai}
%   ... (text) ...
% \textwrittaisep
%   \drawrittai{
...
%   \label{...}
% \end{textwrittai}
\newenvironment{textwrittai}{\begin{minipage}[t]{0.48\textwidth}\vspace{-\topskip}}{\end{minipage}}
\newcommand{\textwrittaisep}{\end{minipage}\hfill\begin{minipage}[t]{0.5\textwidth}\vspace{-\topskip}\centering}
...
% If we externalize rittai, we dont need a switch for it
...
% macro to make two column TOC in book/report document class
% since default behavior put the title of the TOC *inside*
% one column, leaving another column higher than the TOC title
\makeatletter
...
% special formatting for title page since the content list is too long
% and spans over two pages
...
% put \pagestyle after \maketitle since the latter sets thispagestyle to plain
\pagestyle{empty}
...
\chapter*{牌理総合}
\markboth{牌理総合}{} % unnumbered chapters dont mark
\addcontentsline{toc}{chapter}{牌理総合}

\tikzsetfigurename{hairi-figure}
\section{牌効率}
\subfile{section/hairi}

\tikzsetfigurename{teyaku-figure}
\section{手作り}
\subfile{section/teyaku}

\tikzsetfigurename{furo-figure}
\section{副露判断}
...
\chapter*{押し引き}
\markboth{押し引き}{}
\addcontentsline{toc}{chapter}{押し引き}

\tikzsetfigurename{riichi-figure}
\section{立直判断}
\subfile{section/riichi}

\tikzsetfigurename{riichicare-figure}
\section{立直対応}
\subfile{section/riichicare}

\tikzsetfigurename{furocare-figure}
\section{副露対応}
\subfile{section/furocare}

\tikzsetfigurename{read-figure}
\section{河読み}
...
\chapter*{データ集}
\markboth{データ集}{}
\addcontentsline{toc}{chapter}{データ集}
