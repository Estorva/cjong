...
表~\ref{tab:ev-ryanmen-riichi} によると\textbf{3飜50符と跳満確定以外の先制両面は立直有利}。終盤なら4飜30符の7700はダマ有利。例え、手牌~\ref{mj:rch-1-1} の1飜40符を5巡目で立直すると局収支は2800点。一方、ダマにすると局収支は1000点しかないので、早めに先制立直を打つのは大体正解。中盤（8巡目ごろ）では、手牌の価値は \ref{mj:rch-1-1} の3飜50符\footnote{門前加符30符 ＋么九牌暗刻8符＋么九牌暗刻8符＝50符（切り上げ）。詳細な点数計算は\ref{sec:point}参照。}より少ないなら立直するのは大体正解。
...
手牌 \ref{mj:2-4-0} から \ref{mj:2-4-2} までは全て早めの両面リーのみ。打点はただの1300。手牌~\ref{mj:2-4-0} と~\ref{mj:2-4-1} は \mj{22z} を落としたらタンヤオや三色やピンフで打点は大きく上昇するため、速度を遅らせる価値がある。一方、\ref{mj:2-4-2} が \mj{6p} 単騎になる可能性があるので、それを避けて即リーがおすすめ\citep[][pp. 68--73]{tetsusenryaku}。
...
\begin{rittai}[h]
  ...
  \drawrittai{
    jicha/seat = 東,
    jicha/hand = 340m 135p 5066788s -2s,
...
表~\ref{tab:ev-gukei-riichi} によると、3飜40符（ダマで5200、立直で満貫）はダマってもいい。2飜以下は立直有利。手牌~\ref{mj:rch-2-1} のような手牌は立直してもダマしても局収支に大きな差がついていないので和了率優先ならダマにしよう。
...
愚形リーのみの手牌には4つの選択がある：（ア）立直（イ）ダマ（ウ）聴牌外し（エ）アガリ放棄。（エ）の局収支は-1500点ので論外\citep[][p. 30]{miinin-stat}。立直に対して、ダマにすると放銃率は12\%から8\%まで抑えられるが、アガリ率は40\%からから26\%までかなり落ちる。よって、「愚形リーのみ」は決して打つべきでないわけではない\citep{nisi-456kanchan}。また、表~\ref{tab:ev-tsumosen} によると全ての巡目で立直はダマ（ツモだけでアガれるのでツモ専ともいう）より局収支が高い。よって、ダマが有利な局面は超晩巡で形聴を狙う時だけ。
...
\textbf{中盤まで辺張を払えばタンヤオがつくなら聴牌外しは有利}\citep[][pp.~33--36]{miinin-stat}。手牌~\ref{mj:rch-2-2} のような、四連形など強い形がなくても \mj{12p} を払えばタンヤオがあるので聴牌取らない方が有利。ドラが1枚があるなら\citet[p. 35]{miinin-stat}の主張は変わらないが、\citet[p. 165]{rb1}は「即リー」を推奨する。

\tehai{234666m 1288p 4888s}[東1局　南家　8巡目　ドラ \mj{5z}][rch-2-2]

手牌~\ref{mj:rch-2-3} のようなツモ専から \mj{1p} 切ればくっつき一向聴になる。しかし、リーのみと聴牌外しとの和了率の差は5巡目ではすでに5％あって、巡目が進むほどこの差は大きくなる\citep[][p. 34]{miinin-stat}。今回の牌姿は辺張を払ってもメリットは良形になる可能性だけで、打点も上がらない。よって、ここは先制立直をするのは正解。

\tehai{234999m 1277p 4567s}[東1局　南家　5巡目　ドラ \mj{6z}][rch-2-3]
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item 打 \mj{7p} として良形聴牌が \mj{12346p345678s} 11種34枚ある
