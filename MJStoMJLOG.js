// ==UserScript==
// @name         MJS-LOG-DOWNLOADER
// @namespace    mjsld
// @icon
// @version      0.1
// @description  Download log from Mahjong Soul
...
{   //variables you might actually want to change
    ...
    {                      //[kyoku, honba, riichi sticks] - NOTE: 4 mult. works for sanma
        ...
        this.doras       = leaf.dora ? [leaf.dora] : leaf.doras;
        this.draws       = [[],[],[],[]];
        this.discards    = [[],[],[],[]];
        this.actions     = [];
        this.haipais     = this.draws.map( (_, i) => leaf["tiles" + i]);
    ...
    function generatelog(mjslog)
    {
        ...
        mjslog.forEach((e, leafidx) =>
        {
            switch (e.constructor.name)
            {
                ...
                {   //discard - marking riichi
                    let action = {};
                    action["type"] = e.is_liqi ? "R" : "D";
                    action["who"]  = e.seat;
                    action["tile"] = e.tile;

                    if (e.is_liqi) kyoku.nriichi++;
                ...
                {   // call - chi, pon, daiminkan
                    ...
                    switch (e.type)
                    {
                        case 0:
                        {   // chii
                            // tiles[2] is the tile discarded by kami
                            action["type"] = "C";
                            action["tile"] = e.tiles[2];
                            action["furo"] = [e.tiles[0], e.tiles[1]];
                            kyoku.actions.push(action);
                            return;
                        }
                        case 1:
                        {   // pon
                            action["type"] = "P";
                            action["tile"] = e.tiles[0];
                            action["furo"] = [e.tiles[1], e.tiles[2]];
                            kyoku.actions.push(action);
                            return;
                        }
                        case 2:
                        {   // daiminkan
                            action["type"] = "C";
                            action["tile"] = e.tiles[3];
                            action["furo"] = [e.tiles[0], e.tiles[1], e.tiles[2]];
                            kyoku.actions.push(action);
                            kyoku.nkan++;
                ...
                {   // kan - kakan "K", ankan "A"
                    ...
                    switch (e.type)
                    {
                        ...
                        {   // ankan
                            ...
                            action["type"] = "A";
                            action["tile"] = e.tiles;
                            // if an ankan of 5m, 5p, or 5s is made, it must contain aka
                            switch (e.tiles) {
                                case "5m":
                                    action["furo"] = [e.tiles, e.tiles, "0m"];
                                    break;
                                case "5p":
                                    action["furo"] = [e.tiles, e.tiles, "0p"];
                                    break;
                                case "5s":
                                    action["furo"] = [e.tiles, e.tiles, "0s"];
                                    break;
                                default:
                                    action["furo"] = [e.tiles, e.tiles, e.tiles];
                                    break;
                            }
                ...
                case "RecordBaBei" :
                {   // kita - "N" for nuki dora, this record (only) gives {seat, moqie}
    ...
    function parse(record)
    {
        ...
        // Decode actions and put into array `mjslog'
        var mjslog = [];
